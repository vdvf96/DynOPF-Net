...
class OptimalPowerFlow(nn.Module):  
    ...
    def compute_loss(self, x, y, DATA_bounds, DATA_balance, Ybus, c, 
        ...
        delta_pred_dyn = torch.zeros((V_pred.shape[0], n_gen), device=device)
    ...
    def compute_loss_and_stats(self, x, y, DATA_bounds, DATA_balance, Ybus, c, 
        ...
        loss_pred_theta = F.mse_loss(theta_pred, y[: , 2 * self.n_gen + self.n_bus: ])
        loss = loss_pred_p + loss_pred_q + loss_pred_V + loss_pred_theta + c[0] * loss_balance + c[1] * loss_bc + c[2] * loss_instability
        return loss, [loss_pred_p.item(), loss_pred_q.item(), loss_pred_V.item(), loss_pred_theta.item(), loss_balance.item(), loss_bc.item(), loss_instability.item()], torch.cat((Pg_pred, Qg_pred, V_pred, theta_pred),1)
    ...
    def train(self, x_train, y_train, x_valid, y_valid, x_test, y_test, DATA_bounds, DATA_balance, Ybus,
        ...
        self.load_state_dict(torch.load(f'best_model_DYN/model_OPF_{args["id"]}.pth', map_location=torch.device(device)))
        batch_loss_test = []
        with torch.no_grad():
            for x_test, y_test in test_loader:
                loss_test, loss_item, tu, fu, du  = self.compute_loss(x_test, y_test, DATA_bounds, DATA_balance, Ybus, c, NODE_models, t, epoch, -10, args, collected, saved, device)
